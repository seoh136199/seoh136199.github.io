...
### `getAvgScore(int categoryMask, int rerolledCnt, DicesInfo dicesInfo, int rerollMask)`
...
3차원 배열인 `bestAction`을 펼쳐 1차원 배열로 만든 뒤 바이너리 파일에 저장했다. 이 과정을 수행하는 데에도 마찬가지로 5시간 40분이 걸렸다.
